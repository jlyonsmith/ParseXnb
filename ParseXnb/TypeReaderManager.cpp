...
TypeReaderManager::~TypeReaderManager()
{
    ...
    for (vector<GenericTypeReaderFactory*>::iterator iter = genericReaders.begin(); iter != genericReaders.end(); iter++)
    {
    	GenericTypeReaderFactory* factory = *iter;
...
TypeReader* TypeReaderManager::GetByReaderName(wstring const& readerName)
{
    ...
    sprintf(message, "Can't find type reader '%S'.", wanted.c_str());
...
TypeReader* TypeReaderManager::GetByTargetType(wstring const& targetType)
{
    ...
    sprintf(message, "Can't find reader for target type '%S'.", wanted.c_str());
